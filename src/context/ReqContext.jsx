...
const ReqProvider = ({ children }) => {
  ...
  const checkService = (id, serviceBool) => {
    let temp = [...checked];
    let selectedServiceTemp = [...chosenService];
    let obj = servicesData.find((val) => val.id === id);
    // add to cart array
    if (serviceBool === true) {
      temp = [...checked, obj.price];
      selectedServiceTemp = [...chosenService, obj];
      // delete from cart array
    } else {
      temp.splice(checked.indexOf(obj.price), 1);
      selectedServiceTemp.splice(chosenService.indexOf(obj), 1);
    }
    setChecked(temp);
    setChosenService(selectedServiceTemp);
  };

  const serviceSum =
    checked.length !== 0 ? checked.reduce((acc, prev) => acc + prev) : null;
